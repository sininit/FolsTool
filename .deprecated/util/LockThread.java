...
public class LockThread {
	 ...
	 */
	public final <R> Lock execute(Objects.Call<R, Lock> runnable) {
	 ...
	 */
	public final <R> R executeAndJoins(final Objects.Call<R, Lock> runnable) {
		final Value<Throwable> ex = new Value<>();
		final Value<R> rt = new Value<>();
		Objects.Call<R, Lock> lockExecutorValue = new Objects.Call<R, Lock>() {
			@Override
			public R call(Lock param) {
				try {
					return rt.set(runnable.call(param)).get();
				} catch (Throwable e) {
					ex.set(e);
					return null;
				}
	...
	public class Lock extends Thread {
		DoubleLinkedList.Element<Lock> element;
		AtomicReference<Objects.Call<?, Lock>> runnable = new AtomicReference<>();
