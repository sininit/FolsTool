...
public class BytesSeparatorInputStream {
    byte[] content;
    byte[] separators;
    ...
    public BytesSeparatorInputStream(byte[] content) {
        this(content, Finals.Separator.LINE_SEPARATOR_BYTE_N);
    }
    public BytesSeparatorInputStream(byte[] content, byte...   separator) {
    ...
    public byte[] getNext(boolean addSeparator) {
        return getNext(last, addSeparator);
    }
    public byte[] getNext(int offIndex, boolean addSeparator) {
        ...
        byte[] element = sub(offIndex, addSeparator?(offset>=length?offset:offset+1):offset);
        return element;
    }
