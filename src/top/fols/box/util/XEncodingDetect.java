...
public class XEncodingDetect {
	...
	public static final class NotFoundEncoding extends RuntimeException {
		public NotFoundEncoding() {
			super();
		}
		public NotFoundEncoding(java.lang.String message) {
			super(message);
		}
		public NotFoundEncoding(java.lang.String message, java.lang.Throwable cause) {
			super(message, cause);
		}
		public NotFoundEncoding(java.lang.Throwable cause) {
			super(cause);
		}
	}
	...
	public static String getJavaEncode(File bytes) throws IOException, NotFoundEncoding {
		return EncodingType.toJavaEncodingName(new BytesEncodingDetect().detectEncoding(bytes));
	}
	public static Charset getJavaEncode2Charset(File bytes) throws IOException, NotFoundEncoding {
		return forCharsetName(EncodingType.toJavaEncodingName(new BytesEncodingDetect().detectEncoding(bytes)));
	}


	public static String getJavaEncode(byte[] bytes) throws NotFoundEncoding {
		try {
	...
	public static Charset getJavaEncode2Charset(byte[] bytes)throws NotFoundEncoding {
		try {
			return forCharsetName(EncodingType.toJavaEncodingName(new BytesEncodingDetect().detectEncoding(bytes)));
		} catch (IOException e) {
			throw new RuntimeException(e);
	...
	public static String getJavaEncode(InputStream bytes) throws IOException, NotFoundEncoding {
		return EncodingType.toJavaEncodingName(new BytesEncodingDetect().detectEncoding(bytes));
	}
	public static Charset getJavaEncode2Charset(InputStream bytes) throws IOException, NotFoundEncoding {
		return forCharsetName(EncodingType.toJavaEncodingName(new BytesEncodingDetect().detectEncoding(bytes)));
	}



	private static Charset forCharsetName(String name)throws NotFoundEncoding {
		if (null == name)
			throw new NullPointerException();
		try {
			return Charset.forName(name);
		} catch (UnsupportedCharsetException e) {
			throw new NotFoundEncoding(name);
		}
	}
	...
	public static String newString(final byte[] bytes, final int off, final int len) {
		try {
			if (XObjects.isEmpty(bytes))
				return "";
			String encode = EncodingType.toJavaEncodingName(new BytesEncodingDetect().detectEncoding(new XInterfaceSequenceBigByteIO(){
																	@Override
																	public long length() {
	...
	public static class BytesEncodingDetect {
		...
		public EncodingType detectEncoding(File rawtextFile) throws IOException {
			if (rawtextFile.length() == 0)
				return EncodingType.DEAULT;
			byte[] bytes65536 = findResolveBytes(rawtextFile, 8192,  rawtextFile.length());
		...
		public EncodingType detectEncoding(InputStream rawtextFile) throws IOException {
			if (rawtextFile.available() == 0)
				return EncodingType.DEAULT;
			byte[] bytes65536 = findResolveBytes(rawtextFile, 8192, Long.MAX_VALUE);
			XInterfaceSequenceBigByteIO rawtext = XInterfaceSequenceByteUtils.wrapBigByteSequence(bytes65536);
			return detectEncoding(rawtext);
		}
