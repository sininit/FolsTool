...
public class XUnicodeEncoder {
            ...
            if (inUnicode) {
                ...
                if (unicode.size() == 4) {
                    ...
                    if (value == -1) {
                        out.write('\\');
                        out.write('u');
                        out.write(unicode.getBuff(), 0, unicode.size());
                        inUnicode = false;
                        hadSlash = false;
                        continue;
                    }
    ...
    private static void writeHexString(XCharArrayWriter writer, int i) {
        writeUnsignedString(writer, i, 4);
    }

    private static void writeUnsignedString(XCharArrayWriter writer, int i, int shift) {
