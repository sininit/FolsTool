...
public class XHexEncoder {
	...
	public static byte[] encode(byte[] array, int off, int len, boolean upperCase) throws OutOfMemoryError, ArrayIndexOutOfBoundsException {
		...
		putBytesToHex(array, off, len, result, 0, true);
		return result;
	}
	...
	public static void putBytesToHex(byte[] array, int off, int len, 
		...
		byte[] HEX_CHAR = upperCase ? XHexEncoder.HEX_CHAR_BYTES_UPPERCASE: XHexEncoder.HEX_CHAR_BYTES_LOWERCASE;
		for (int i = 0;i < len;i++) {
			byte b = array[i + off];
			int a;
			if (b < 0) {
