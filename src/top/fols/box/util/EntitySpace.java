...
@SuppressWarnings({"SpellCheckingInspection", "unchecked", "rawtypes"})
public abstract class EntitySpace<K, E extends EntitySpace.Entity> {
    ...
    public static class Entity<K> {
		private EntitySpace $$master;
        private Entity      $$parent;
		
        private K      		$id;
        private Set<Entity> $element = new LinkedHashSet<>();

        public Entity(EntitySpace master, K key) {
			this.$$master = master;
		...
		public EntitySpace getTable() { return $$master; }

		
        public Entity        getParent()      { return $$parent;}
        public Set<Entity>   getElements()    { return new LinkedHashSet<>(getElementsInternal()); }
	...
	void 	checkFromMap(Map<String, Object> source, E writer) {
		...
		if (null != className) {
			...
			if (!checkFromMapEntityType(clazz, writer)) {
    ...
    void removeEntiryMapId(E entity) {
        E   remove = entityMap.remove(getId(entity));
        if (remove != null) {
	...
	private final WeakLevel3Cache<String, Class, ClassNotFoundException> classForNameCache = new WeakLevel3Cache<String, Class, ClassNotFoundException>(){
		@Override
	...
	private abstract static class WeakLevel3Cache<K, V, EX extends Throwable> {
