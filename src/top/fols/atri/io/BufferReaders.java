...
public class BufferReaders<T extends Reader> extends Reader implements Delimiter.ICharsDelimiterReaderIO, IInnerStream<T> {
	 ...
	 * 假设 forceRead = false; 那么 读完 available 后就会退出不再读取(BufferedReader就是这样的)
			...
			if (buffCount <= 0) {// no buff
				...
				if (buff.length  >= defaultBufferSizeM3) {
					if (buffCount < defaultBufferSize) {
						char[] data = new char[defaultBufferSize];
...
//	do {
//		char[] next = buff.readNextLine(false);
//		if (null == next)
//			break;
...
//	} while (buff.hasNext());
	...
	public char[] readNext() throws IOException {
		...
		if (last == offset) {
			...
			//no avail data
			char[] array = new char[limit - last];
			Arrayz.arraycopy(data, last, array, 0, array.length);
			int v = array.length;
			this.buffIndex += v;
			this.buffCount -= v;
			...
			while (offset < limit) {
				...
				if (delimiter.isHead(datum)) {
					...
					if (sIndex > -1) {
						...
						char[] array = new char[(offset - last) + (resultAddSeparator ? separators[sIndex].length: 0)];
						if (array.length > 0)
							Arrayz.arraycopy(data, last, array, 0, array.length);
						this.separatorIndex = sIndex;
						this.buffIndex += v;
	...
	public String readNextLineAsString(boolean resultAddSeparator) throws IOException {
		char[] bytes = readNextLine(resultAddSeparator);
		return null == bytes ? null : newString(bytes);
	}
	public String newString(char[] data) {
