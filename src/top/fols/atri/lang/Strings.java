...
public class Strings {
	...
	public static final String LETTERS_UPPERCASE = LETTERS_LOWERCASE.toUpperCase();
	...
	public static String repeat(String str, int repeatLength) {
		...
		if (strlen == 0 || repeatLength == 0) { return Finals.STRING_EMPTY_VALUE; }

		if (repeatLength == 1) {
			return str;
		}
		if (repeatLength == 2) {
			char[] buffer = new char[strlen * repeatLength];
			Arrayz.arraycopy(str, 0, buffer, 0, strlen);
			Arrayz.arraycopy(str, 0, buffer, strlen, strlen);
			return new String(buffer);
		}

		return new String(Arrayx.repeat(str.toCharArray(), repeatLength));
	}
	...
	public static String padStart(String input, int minLength, String paddingString) {
		return padString(input, minLength, paddingString, true);
	}
	public static String padEnd(String input, int minLength, String paddingString) {
		return padString(input, minLength, paddingString, false);
	}
	static String padString(String input, int minLength, String paddingString, boolean isLeft) {
		...
		int pstrlen = paddingString.length();
		int need = minLength - inputLength;
		if (need <= pstrlen) {
			if (isLeft) {
			...
			int pc = IndexPieces.getPieceCount(need, pstrlen);
			String v = repeat(paddingString, pc);
			if (isLeft) {
				return v + input;
			} else {
				return input + v;
	...
	public static String randomLowercaseLetters(int length) {return random(LETTERS_LOWERCASE, length);}
	public static String randomUppercaseLetters(int length) {return random(LETTERS_UPPERCASE, length);}
	...
	public static String retain(CharSequence str, int off, int len, char[] retain) {
		...
		String string = retains.toString(); retains.close();
		return string;
	}
	...
	public static int 			searchRepeatCount(String str, String find) {
		if ((null == str  || str.length() == 0) || (null == find || find.length() == 0)) {
			return 0;
		}
		int i = 0;
