...
public class ArrayLists<E> extends AbstractList<E>
    ...
    @Private
    private ArrayLists(E[] old, boolean isClone) {
        int length = old.length;
        if (length == 0) {
            this.elementData = this.emptyData = old;
        } else {
            this.elementData = isClone ? old.clone() : old;
    ...
    @Private
    private E[] trimToSizeAndGetBuffer() {
    ...
    //buffer.length >= size
    public void value(int index, E element) {
        elementData[index] = element;
    }
    ...
    public boolean addAll(Collection<? extends E> c) {
        return append(c.toArray());
    }
    public boolean addAll(E[] a) {
        return addAll(a);
    }
    private boolean append(Object[] a) {
        int numNew = a.length;
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        return append(index, c.toArray());
    }
    public boolean addAll(int index, E[] c) {
        return append(index, c);
    }
    private boolean append(int index, Object[] a) {
        rangeCheckForAdd(index);
    ...
    public boolean addAll(int index, int count, E fill) {
        rangeCheckForAdd(index);
        if (count <= 0)
            return false;
    ...
    public static <E> ArrayLists<E> fromArray(E[] v) {
        return fromArray(v);
    }
    public static <E> ArrayLists<E> fromArray(Object val) {
        if (val == null)
            return null;
        Class type, component;
        if  ((type = val.getClass()).isArray()) {
            if ((component = type.getComponentType()).isPrimitive()) {
                return new ArrayLists<E>((E[]) Arrayx.convertElement(val, Boxing.toWrapperType(component)));
            } else {
                return new ArrayLists<E>((E[]) val);
            }
        }
    ...
    @Private
    public static class Unsafe<E> {
        ArrayLists<E> this$0;
        private Unsafe(ArrayLists<E> master) {
            this.this$0 = master;
        }

        public E[]  data()  { return this$0.elementData; }
        public void data(E[] val) { if (null == val) throw new UnsupportedOperationException("data is null");this$0.elementData = val;}

        public int  size() { return this$0.size(); }
        public void size(int v) { this$0.setSize(v); }

        @Private
        public E[] trimToSizeAndGetBuffer() {
            return this.this$0.trimToSizeAndGetBuffer();
        }
         ...
         */
        @Private
        public static <E> ArrayLists<E> trusteeship(E[] arr) {
            return new ArrayLists<E>(arr, false);
        }
    }
    protected transient Unsafe<E> unsafe;
    @Private
    protected Unsafe<E> unsafe() {
        Unsafe<E> u;
        if (null == (u = this.unsafe))
            this.unsafe = u = new Unsafe<>(this);
        return u;
    }
}
