...
public class DiffMatchPatch {
    ...
    public static class SimpleDiff {
        ...
        public static class DiffEntity {
            ...
            public DiffOperation getDiffOperation() {return diffOperation;}

            public String getOldText() {return old;}
            public String getText() {return text;}
        }
        List<DiffEntity> entities = new ArrayList<>();
        public List<DiffEntity> getEntities() {
            return entities;
        }
    }
    public SimpleDiff toSimpleDiff(LinkedList<Diff> diffs) {
        SimpleDiff simpleDiff = new SimpleDiff();
        Diff last = null;
        for (Diff diff : diffs) {
            SimpleDiff.DiffEntity diffEntity = new SimpleDiff.DiffEntity();
            $: {
                if (null != last) {
                    if (last.operation == Operation.DELETE) {
                        if (diff.operation == Operation.INSERT) {
                            diffEntity.diffOperation = SimpleDiff.DiffOperation.CHANGE;
                            diffEntity.old  = last.text;
                            diffEntity.text = diff.text;

                            simpleDiff.entities.remove(simpleDiff.entities.size() - 1);
                            simpleDiff.entities.add(diffEntity);
                            break $;
                        }
                    }
                }
                ...
                switch (diff.operation) {
                    case EQUAL:
                        diffEntity.diffOperation = SimpleDiff.DiffOperation.EQUAL;
                        diffEntity.old  = diff.text;
                        break;
                    case INSERT:
                        diffEntity.diffOperation = SimpleDiff.DiffOperation.INSERT;
                        diffEntity.old  = null;
                        break;
                    case DELETE:
                        diffEntity.diffOperation = SimpleDiff.DiffOperation.DELETE;
                        diffEntity.old  = diff.text;
                        break;
                }
                diffEntity.text = diff.text;
    ...
    public static void main(String[] args){
        ...
        String text1 =
                "1.受理责任（法规督查处）：公示依法应当提交的材料；一次性告知补正材料；依法受理或不予受理（不予受理应当告知理由）。\n" +
                        "2.审查责任（法规督查处）：组织书面审查、现场检查。\n" +
                        "3.决定责任（法规督查处）：根据书面审查、现场审查情况，作出是否授予资质的决定（不授予资质的，书面告知理由，并告知申请人享有依法申请行政复议或提起行政诉讼的权利）。\n" +
                        "4.送达责任（法规督查处）：制发并送达审批决定书，准予许可的，同时颁发资质证书。\n" +
                        "5.事后监管责任（法规督查处）：对取得资质的单位进行监督管理，组织开展保密教育培训和保密检查。\n" +
                        "6.法律法规规章文件规定的其他应履行的责任（有关处室）。\n";
        String text2 =
                "1.受理责任（科学技术处）：公示依法应当提交的材料；一次性告知补正材料；依法受理或不予受理（不予受理应当告知理由）。\n" +
                        "2.审查责任（科学技术处）：组织书面审查、现场检查、专家评审。\n" +
                        "3.决定责任（科学技术处）：根据书面审查、现场审查和专家评审情况，作出是否授予资质的决定（不授予资质的，书面告知理由，并告知申请人享有依法申请行政复议或提起行政诉讼的权利）。\n" +
                        "4.送达责任（科学技术处）：制发并送达审批决定书，准予许可的，同时颁发资质证书。\n" +
                        "5.事后监管责任（科学技术处）：对取得资质的单位进行监督管理，组织开展保密教育培训和保密检查。\n" +
                        "6.法律法规规章文件规定的其他应履行的责任（有关处室）。\n";
