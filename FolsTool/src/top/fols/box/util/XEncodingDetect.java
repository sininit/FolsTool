...
// and have expected frequency of use
			...
			for (i = 0; i < rawtextlen - 1; i++)
			{
				if (rawtext.byteAt(i) == (byte) 0x1B && i + 3 < rawtextlen)
				{ // Escape char ESC
					if (rawtext.byteAt(i + 1) == (byte) 0x24 && rawtext.byteAt(i + 2) == 0x29 && rawtext.byteAt(i + 3) == (byte) 0x41)
					{ // GB Escape $ ) A
						i += 4;
...
// CNS Escape $ ) G
					...
					if (rawtext.byteAt(i) == (byte) 0x1B && i + 2 < rawtextlen && rawtext.byteAt(i + 1) == (byte) 0x28 && rawtext.byteAt(i + 2) == (byte) 0x42)
					{ // ASCII:
// ESC
// ( B
						i += 2;
...
// Check to see if characters fit into acceptable ranges
			...
			for (i = 0; i < rawtextlen; i++)
			{
				if ((rawtext.byteAt(i) & (byte) 0x7F) == rawtext.byteAt(i))
				{ // One byte
					asciibytes++;
...
// Allows for some (few) bad formed sequences
		...
		int utf16_probability(XInterfaceSequenceIOBigByte rawtext) throws IOException
		{
			if (rawtext.length() > 1 && ((byte) 0xFE == rawtext.byteAt(0) && (byte) 0xFF == rawtext.byteAt(1)) || // Big-endian
				((byte) 0xFF == rawtext.byteAt(0) && (byte) 0xFE == rawtext.byteAt(1)))
			{ // Little-endian
				return 100;
			}
			return 0;
		...
		int ascii_probability(XInterfaceSequenceIOBigByte rawtext) throws IOException
		{
			...
			for (i = 0; i < rawtextlen; i++)
			{
				...
				else if (rawtext.byteAt(i) == (byte) 0x1B)
